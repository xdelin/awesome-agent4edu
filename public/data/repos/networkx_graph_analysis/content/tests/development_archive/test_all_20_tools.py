#!/usr/bin/env python3
"""Comprehensive test of all 20 tools in the NetworkX MCP server."""
...
class ComprehensiveToolTester:
    ...
    def test_connected_components(self):
        ...
        if "result" in response and not response["result"].get("isError"):
            ...
            return "components" in result and "num_components" in result
        return False
    ...
    def test_pagerank(self):
        ...
        if "result" in response and not response["result"].get("isError"):
            result = json.loads(response["result"]["content"][0]["text"])
            print(f"PageRank top 5: {list(result.items())[:5]}")
            return "A" in result and isinstance(result["A"], float)
        return False
...
Z,X,3.0"""
    ...
    def test_analyze_author_impact(self):
        ...
        response = self.send_request(
            "tools/call",
            {"name": "analyze_author_impact", "arguments": {"graph": "author_graph"}},
        )
    ...
    def test_find_collaboration_patterns(self):
        ...
        response = self.send_request(
            "tools/call",
            {
                ...
                "arguments": {"graph": "author_graph", "min_collaborations": 1},
            },
        )
        ...
        if "result" in response and not response["result"].get("isError"):
            result = json.loads(response["result"]["content"][0]["text"])
            print(
                f"Collaboration patterns: {result.get('total_collaborators', 0)} collaborators"
            )
            return "patterns" in result
        return False
    ...
    def test_detect_research_trends(self):
        ...
        if "result" in response and not response["result"].get("isError"):
            result = json.loads(response["result"]["content"][0]["text"])
            print(f"Research trends: {result.get('total_years', 0)} years analyzed")
    ...
    def test_export_bibtex(self):
        ...
        try:
            response = self.send_request(
                ...
                {
                    "name": "export_bibtex",
                    "arguments": {"graph": "author_graph", "output_path": temp_path},
                },
            )

            if "result" in response and not response["result"].get("isError"):
                result = json.loads(response["result"]["content"][0]["text"])
                print(f"BibTeX export: {result}")
                # Check if file exists
                file_exists = os.path.exists(temp_path)
                print(f"File created: {'✅' if file_exists else '❌'}")
                return "exported" in result
        finally:
    ...
    def test_recommend_papers(self):
        ...
        if "result" in response and not response["result"].get("isError"):
            ...
            for rec in result.get("recommendations", [])[:3]:
                print(f"  - {rec['paper']} (score: {rec['score']})")
    ...
    def run_all_tests(self):
        ...
        print("🧪 COMPREHENSIVE TEST OF ALL 20 TOOLS 🧪")
        print("=" * 50)
