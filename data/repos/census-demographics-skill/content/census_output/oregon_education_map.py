...
or higher by county in Oregon using US Census ACS 5-Year data.
"""
...
def fetch_education_data():
    """
    Fetch educational attainment data for Oregon counties from Census API.
    ...
    try:
        ...
        df['county_name'] = df['NAME'].str.replace(' County, Oregon', '')
        ...
        print(f"Successfully fetched data for {len(df)} Oregon counties")
...
def create_map(df, output_dir):
    ...
    try:
        ...
        oregon_counties['full_fips'] = oregon_counties['STATEFP'] + oregon_counties['COUNTYFP']

        # Merge with education data
        oregon_map = oregon_counties.merge(df, on='full_fips', how='left')
        ...
        # Add county labels
        oregon_map['coords'] = oregon_map['geometry'].apply(
        ...
        # Save the map
        map_file = os.path.join(output_dir, 'oregon_education_map.png')
...
def save_data(df, output_dir):
    ...
    # Save to CSV
    csv_file = os.path.join(output_dir, 'oregon_education_data.csv')
    df_sorted.to_csv(csv_file, index=False)
    print(f"CSV saved to: {csv_file}")

    # Save to JSON
    json_file = os.path.join(output_dir, 'oregon_education_data.json')
...
def main():
    ...
    output_dir = f"oregon_education_{timestamp}"
